...
<div class="py-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="space-y-5">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                ...
                <div class="space-y-3 text-sm">
                    <div class="flex justify-between text-gray-600">
                        ...
                        <span class="font-mono font-semibold">Rp {{ number_format($transaksi->total_tagihan ?? $transaksi->total_kotor, 0, ',', '.') }}</span>
                    </div>
                    
                    @if($transaksi->status_bayar === 'lunas')
                        <div class="flex justify-between text-gray-600">
                            <span>Uang Diterima</span>
                            <span class="font-mono">Rp {{ number_format($transaksi->uang_diterima ?? 0, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between text-gray-600">
                            <span>Kembalian</span>
                            <span class="font-mono text-blue-600">Rp {{ number_format($transaksi->kembalian ?? 0, 0, ',', '.') }}</span>
                        </div>
                        <div class="border-t pt-2 flex justify-between font-bold text-green-700 text-base">
                            <span>Status</span>
                    ...
                    @else
                        <div class="flex justify-between text-gray-600">
                            ...
                            <span class="font-mono text-green-600">Rp {{ number_format($transaksi->total_dibayar ?? 0, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between text-gray-600">
                            <span>Sisa Tagihan</span>
                            <span class="font-mono font-bold text-red-600">Rp {{ number_format($transaksi->sisa_tagihan ?? ($transaksi->total_tagihan ?? $transaksi->total_kotor), 0, ',', '.') }}</span>
                        </div>
                        <div class="border-t pt-3 space-y-2">
                            <a href="{{ route('pembayaran.show', $transaksi) }}" 
                               class="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white py-3 px-4 rounded-lg font-semibold transition-colors">
                                <i class="fa-solid fa-history"></i>
                                Riwayat Pembayaran
                            </a>
                            <a href="{{ route('pembayaran.create', ['transaksi' => $transaksi->id]) }}" 
                               class="w-full flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white py-3 px-4 rounded-lg font-semibold transition-colors">
            ...
            @if($transaksi->pembayaran && $transaksi->pembayaran->count())
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                <h2 class="font-semibold text-gray-700 mb-3 flex items-center gap-2">
                    <i class="fa-solid fa-receipt text-purple-500"></i> Riwayat Pembayaran
                </h2>
                <div class="space-y-2">
                    @foreach($transaksi->pembayaran->sortByDesc('created_at') as $bayar)
                    <div class="flex justify-between items-center text-sm py-2 border-b last:border-b-0">
                        <div>
                            <div class="font-medium text-gray-700">{{ $bayar->kode_pembayaran }}</div>
                ...
                <div class="mt-3 pt-3 border-t text-center">
                    <span class="inline-flex items-center gap-1 bg-green-100 text-green-800 px-4 py-2 rounded-full font-semibold">
